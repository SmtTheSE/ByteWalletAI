...
> Hybrid custom ML model + local Ollama LLM — privacy-first, no cloud required.
...
- **`ai_message`** — 2-3 sentence coaching message generated by a local Ollama LLM
...
│                                                           ▼
│  3. LLM Service  ─────────────► Ollama (local, private)
│     (app/services/llm_service.py)  model: llama3.2
│                                   No PII sent. Runs on-device.
│                                                           ▼
...
**Privacy:** All inference runs locally (Ollama). No financial data leaves the device.
